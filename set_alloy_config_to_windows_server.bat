...
set "MSG_HOST_INFO_START=[DEPLOY] Generating host_info.json (UUID/GUID)..."
set "MSG_HOST_INFO_OK=[OK] Host Info JSON created."
set "MSG_HOST_INFO_FAIL=[ERROR] Failed to create Host Info JSON."
...
set "MSG_HOST_INFO_START=[部署] 正在產生 host_info.json (UUID/GUID)..."
set "MSG_HOST_INFO_OK=[OK] Host Info JSON 已建立。"
set "MSG_HOST_INFO_FAIL=[錯誤] 建立 Host Info JSON 失敗。"
...
:: --- [INTEGRATION] Generate Host Info JSON (Corrected) ---
...
if exist "%TARGET_DIR%\host_info.json" (
    ...
    echo %C_RED%%MSG_HOST_INFO_FAIL%%C_RESET%
    echo Warning: Configuration will fail because 'host_info.json' is missing.
)
:: ---------------------------------------------
